...
class MuJsonLoader(object):
    ...
    def save(self, path):
        if self.json is not None:
            output = json.dumps(
                self.json, sort_keys=True, indent=4, separators=(",", ": ")
            )
            with open(path, "a"):
                pass
            with open(path, "rb+") as f:
                ...
                for row in self.data.json:
                    if int(row["port"]) == muid:
                        ...
                        protocol_param = "/?protoparam=" + common.to_str(
                            base64.urlsafe_b64encode(common.to_bytes(param))
                        ).replace("=", "")
                        break
        link = (
            "%s:%s:%s:%s:%s:%s"
            % (
                ...
                obfs,
                common.to_str(
                    base64.urlsafe_b64encode(common.to_bytes(user["passwd"]))
                ).replace("=", ""),
            )
        ) + protocol_param
        return "ssr://" + (
            encode
            and common.to_str(base64.urlsafe_b64encode(common.to_bytes(link))).replace(
                "=", ""
            )
            or link
        )
