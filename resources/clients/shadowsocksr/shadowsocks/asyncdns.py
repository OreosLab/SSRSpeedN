...
VALID_HOSTNAME = re.compile(rb"(?!-)[A-Z\d_-]{1,63}(?<!-)$", re.IGNORECASE)
...
class DNSResolver(object):
    ...
    def _handle_data(self, data):
        ...
        if response and response.hostname:
            ...
            if IPV6_CONNECTION_SUPPORT:
                if (
                    not ip
                    and self._hostname_status.get(hostname, STATUS_IPV4) == STATUS_IPV6
                ):
                ...
                if (
                    not ip
                    and self._hostname_status.get(hostname, STATUS_IPV6) == STATUS_IPV4
                ):
    ...
    def _send_req(self, hostname, qtype):
        ...
        for server in self._servers:
            logging.debug(
                "resolving %s with type %d using server %s", hostname, qtype, server
            )
        ...
        if not hostname:
            ...
            if False:
                addrs = socket.getaddrinfo(
                    hostname, 0, 0, socket.SOCK_DGRAM, socket.SOL_UDP
                )
                if addrs:
