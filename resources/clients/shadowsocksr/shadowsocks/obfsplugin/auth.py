...
from shadowsocks import common, encrypt, lru_cache
...
class obfs_auth_v2_data(object):
    ...
    def insert(self, client_id, connection_id):
        if (
            self.client_id.get(client_id, None) is None
            or not self.client_id[client_id].enable
        ):
...
class auth_sha1_v4(auth_base):
    ...
    def pack_auth_data(self, buf):
        ...
        crc = (
            binascii.crc32(
                struct.pack(">H", data_len) + self.salt + self.server_info.key
            )
            & 0xFFFFFFFF
        )
        ...
        data = struct.pack(">H", data_len) + data
        data += hmac.new(
            self.server_info.iv + self.server_info.key, data, hashlib.sha1
        ).digest()[:10]
        return data
    ...
    def auth_data(self):
        ...
        if not self.server_info.data.local_client_id:
            ...
            self.server_info.data.connection_id = (
                struct.unpack("<I", os.urandom(4))[0] & 0xFFFFFF
            )
    ...
    def client_post_decrypt(self, buf):
        ...
        while len(self.recv_buf) > 4:
            ...
            if (
                struct.pack(
                    "<I", zlib.adler32(self.recv_buf[: length - 4]) & 0xFFFFFFFF
                )
                != self.recv_buf[length - 4 : length]
            ):
                self.raw_trans = True
                self.recv_buf = b""
    ...
    def server_post_decrypt(self, buf):
        ...
        if not self.has_recv_header:
            ...
            crc = struct.pack(
                "<I",
                binascii.crc32(self.recv_buf[:2] + self.salt + self.server_info.key)
                & 0xFFFFFFFF,
            )
            ...
            if time_dif < -self.max_time_dif or time_dif > self.max_time_dif:
                ...
                logging.info(
                    "auth_sha1_v4: auth fail, data %s" % (binascii.hexlify(out_buf),)
                )
                return self.not_match_return(self.recv_buf)
            self.recv_buf = self.recv_buf[length:]
            self.has_recv_header = True
        ...
        while len(self.recv_buf) > 4:
            ...
            if (
                ...
                logging.info(
                    "auth_sha1_v4: checksum error, data %s"
...
class obfs_auth_mu_data(object):
    ...
    def insert(self, user_id, client_id, connection_id):
        ...
        if (
            local_client_id.get(client_id, None) is None
            or not local_client_id[client_id].enable
        ):
            if (
                local_client_id.first() is None
                or len(local_client_id) < self.max_client
            ):
...
class auth_aes128_sha1(auth_base):
    def __init__(self, method, hashfunc):
        ...
        self.max_time_dif = 60 * 60 * 24  # time dif (second) setting
        self.salt = (
            hashfunc == hashlib.md5 and b"auth_aes128_md5" or b"auth_aes128_sha1"
        )
        self.no_compatible_method = (
    ...
    def rnd_data(self, buf_size, full_buf_size):
        ...
        return (
            common.chr(255) + struct.pack("<H", data_len + 1) + os.urandom(data_len - 2)
        )
    ...
    def client_post_decrypt(self, buf):
        ...
        while len(self.recv_buf) > 4:
            ...
            if (
                hmac.new(mac_key, self.recv_buf[: length - 4], self.hashfunc).digest()[
                    :4
                ]
                != self.recv_buf[length - 4 : length]
            ):
    ...
    def server_post_decrypt(self, buf):
        ...
        if not self.has_recv_header:
            if len(self.recv_buf) >= 7 or len(self.recv_buf) in [2, 3]:
                ...
                sha1data = hmac.new(mac_key, self.recv_buf[:1], self.hashfunc).digest()[
                    : recv_len - 1
                ]
            ...
            if len(self.recv_buf) < 31:
                return (b"", False)
            sha1data = hmac.new(mac_key, self.recv_buf[7:27], self.hashfunc).digest()[
                :4
            ]
            ...
            head = encryptor.decrypt(
                b"\x00" * 16 + self.recv_buf[11:27] + b"\x00"
            ...
            if (
                hmac.new(
                    self.user_key, self.recv_buf[: length - 4], self.hashfunc
                ).digest()[:4]
                != self.recv_buf[length - 4 : length]
            ):
                logging.info(
