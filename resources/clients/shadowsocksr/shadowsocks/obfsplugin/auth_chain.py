...
class auth_chain_a(auth_base):
    ...
    def pack_auth_data(self, auth_data, buf):
        ...
        uid = (
            struct.unpack("<I", uid)[0]
            ^ struct.unpack("<I", self.last_client_hash[8:12])[0]
        )
        ...
        self.encryptor = encrypt.Encryptor(
            ...
            + to_bytes(base64.b64encode(self.last_client_hash)),
            "rc4",
        )
    ...
    def client_post_decrypt(self, buf):
        ...
        while len(self.recv_buf) > 4:
            ...
            data_len = (
                struct.unpack("<H", self.recv_buf[:2])[0]
                ^ struct.unpack("<H", self.last_server_hash[14:16])[0]
            )
            ...
            server_hash = hmac.new(
                mac_key, self.recv_buf[: length + 2], self.hashfunc
            ).digest()
    ...
    def server_pre_encrypt(self, buf):
        ...
        if self.pack_id == 1:
            tcp_mss = (
                self.server_info.tcp_mss if self.server_info.tcp_mss < 1500 else 1500
            )
    ...
    def server_post_decrypt(self, buf):
        ...
        if not self.has_recv_header:
            ...
            uid = (
                struct.unpack("<I", self.recv_buf[12:16])[0]
                ^ struct.unpack("<I", md5data[8:12])[0]
            )
            ...
            md5data = hmac.new(
                self.user_key, self.recv_buf[12 : 12 + 20], self.hashfunc
            ).digest()
        ...
        while len(self.recv_buf) > 4:
            ...
            data_len = (
                struct.unpack("<H", self.recv_buf[:2])[0]
                ^ struct.unpack("<H", self.last_client_hash[14:16])[0]
            )
            rand_len = self.rnd_data_len(
    ...
    def client_udp_pre_encrypt(self, buf):
        ...
        encryptor = encrypt.Encryptor(
            to_bytes(base64.b64encode(self.user_key))
            + to_bytes(base64.b64encode(md5data)),
            "rc4",
        )
    ...
    def server_udp_pre_encrypt(self, buf, uid):
        ...
        rand_len = self.udp_rnd_data_len(md5data, self.random_server)
        encryptor = encrypt.Encryptor(
            to_bytes(base64.b64encode(user_key)) + to_bytes(base64.b64encode(md5data)),
            "rc4",
        )
        out_buf = encryptor.encrypt(buf)
...
class auth_chain_b(auth_chain_a):
    ...
    def rnd_data_len(self, buf_size, last_hash, random):
        ...
        pos = bisect.bisect_left(
            self.data_size_list, buf_size + self.server_info.overhead
        )
        ...
        pos = bisect.bisect_left(
            self.data_size_list2, buf_size + self.server_info.overhead
        )
        final_pos = pos + random.next() % (len(self.data_size_list2))
        if final_pos < len(self.data_size_list2):
            return (
                self.data_size_list2[final_pos] - buf_size - self.server_info.overhead
            )
