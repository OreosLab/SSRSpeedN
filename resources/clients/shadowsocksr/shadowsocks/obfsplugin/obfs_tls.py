...
class tls_ticket_auth(plain.plain):
    ...
    def pack_auth_data(self, client_id):
        ...
        data += hmac.new(self.server_info.key + client_id, data, hashlib.sha1).digest()[
            :10
        ]
        return data
    ...
    def client_encode(self, buf):
        ...
        if len(buf) > 0:
            self.send_buffer += (
                b"\x17" + self.tls_version + struct.pack(">H", len(buf)) + buf
            )
        if self.handshake_status == 0:
            ...
            data = (
                ...
                + binascii.unhexlify(
                    b"001cc02bc02fcca9cca8cc14cc13c00ac014c009c013009c0035002f000a"
                    + b"0100"
                )
            )
            ...
            if host not in self.server_info.data.ticket_buf:
                self.server_info.data.ticket_buf[host] = os.urandom(
                    (struct.unpack(">H", os.urandom(2))[0] % 17 + 8) * 16
                )
            ext += (
                b"\x00\x23"
                + struct.pack(">H", len(self.server_info.data.ticket_buf[host]))
                + self.server_info.data.ticket_buf[host]
            )
            ...
            data += (
                b"\x16" + self.tls_version + b"\x00\x20" + os.urandom(22)
            )  # Finished
            data += hmac.new(
                self.server_info.key + self.server_info.data.client_id,
                data,
                hashlib.sha1,
            ).digest()[:10]
    ...
    def server_encode(self, buf):
        ...
        data = (
            self.tls_version
            + self.pack_auth_data(self.client_id)
            + b"\x20"
            + self.client_id
        ...
        if random.randint(0, 8) < 1:
            ...
            ticket = (
                struct.pack(">H", len(ticket) + 4)
                + b"\x04\x00"
                + struct.pack(">H", len(ticket))
                + ticket
            )
        ...
        data += hmac.new(
            self.server_info.key + self.client_id, data, hashlib.sha1
        ).digest()[:10]
        if buf:
    ...
    def server_decode(self, buf):
        ...
        if (self.handshake_status & 1) == 1:
            ...
            if not match_begin(
                buf, b"\x14" + self.tls_version + b"\x00\x01\x01"
            ):  # ChangeCipherSpec
                raise Exception("server_decode data error")
            buf = buf[6:]
            ...
            if (
                hmac.new(
                    self.server_info.key + self.client_id,
                    verify[:verify_len],
                    hashlib.sha1,
                ).digest()[:10]
                != verify[verify_len : verify_len + 10]
            ):
                raise Exception("server_decode data error")
        ...
        sha1 = hmac.new(
            self.server_info.key + sessionid, verifyid[:22], hashlib.sha1
        ).digest()[:10]
        ...
        if self.server_info.obfs_param:
            try:
                ...
                pass
        if self.max_time_dif > 0 and (
            time_dif < -self.max_time_dif
            or time_dif > self.max_time_dif
            or common.int32(utc_time - self.server_info.data.startup_time)
            < -self.max_time_dif / 2
        ):
