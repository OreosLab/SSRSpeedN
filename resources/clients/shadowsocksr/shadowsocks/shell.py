...
def check_config(config, is_local):
    ...
    if config.get("timeout", 300) < 100:
        logging.warning(
            "warning: your timeout %d seems too short" % int(config.get("timeout"))
        )
    if config.get("timeout", 300) > 600:
        logging.warning(
            "warning: your timeout %d seems too long" % int(config.get("timeout"))
        )
    if config.get("password") in [b"mypassword"]:
        logging.error(
            "DON'T USE DEFAULT PASSWORD! Please change it in your " "config.json!"
        )
        sys.exit(1)
...
def get_config(is_local):
    ...
    if is_local:
        ...
        try:
            ...
            sys.exit(2)
        try:
            config["ignore_bind"] = IPNetwork(
                config.get(
                    "ignore_bind", "127.0.0.0/8,::1/128,10.0.0.0/8,192.168.0.0/16"
                )
            )
        except Exception as e:
            logging.error(e)
