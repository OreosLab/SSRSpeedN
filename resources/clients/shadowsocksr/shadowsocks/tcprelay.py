...
from shadowsocks.common import parse_header, pre_parse_header
...
class TCPRelayHandler(object):
    def __init__(
        self, server, fd_to_handlers, loop, local_sock, config, dns_resolver, is_local
    ):
        self._server = server
        ...
        self._overhead = self._obfs.get_overhead(
            self._is_local
        ) + self._protocol.get_overhead(self._is_local)
                ...
                while len(self._udp_data_send_buffer) > 6:
                    ...
                    if (addrtype & 7) == 3:
                        ...
                        if af == False:
                            ...
                            return self._handle_server_dns_resolved(
                                "",
                                (dest_addr, dest_port),
                                dest_addr,
                                data[header_length:],
                        ...
                        return self._handle_server_dns_resolved(
                            "", (dest_addr, dest_port), dest_addr, data[header_length:]
                        )
            ...
            for host in host_list:
                ...
                if len(items_match) > 1:
                    if items_match[1] != "*":
                        try:
                            if (
                                self._server._listen_port != int(items_match[1])
                                and int(items_match[1]) != 0
                            ):
                                continue
                        except:
                            pass
                ...
                if (
                    items_match[0] != "*"
                    and common.match_regex(items_match[0], ogn_data) == False
                ):
                    continue
                if len(items) > 1:
                    try:
    ...
    def _handel_protocol_error(self, client_address, ogn_data):
        logging.warn(
            "Protocol ERROR, TCP ogn data %s from %s:%d via port %d by UID %d"
            % (
        ...
        data = (
            b"\x03"
            + common.to_bytes(common.chr(len(host)))
            + common.to_bytes(host)
            + struct.pack(">H", port)
        )
    ...
    def _handle_stage_connecting(self, data):
        ...
        if (
            self._is_local
            and not self._fastopen_connected
            and self._config["fast_open"]
        ):
        ...
        if not self._remote_udp and not self._is_redirect:
            if self._forbidden_iplist:
                if common.to_str(sa[0]) in self._forbidden_iplist:
                    if self._remote_address:
                        raise Exception(
                            "IP %s is in forbidden list, when connect to %s:%d via port %d by UID %d"
                            % (
                                common.to_str(sa[0]),
            ...
            if self._forbidden_portset:
                if sa[1] in self._forbidden_portset:
                    if self._remote_address:
                        raise Exception(
                            "Port %d is in forbidden list, when connect to %s:%d via port %d by UID %d"
                            % (
                                sa[1],
                                self._remote_address[0],
                                self._remote_address[1],
                    ...
                    if self._is_local and self._config["fast_open"]:
                        ...
                        if self._remote_udp:
                            ...
                            common.connect_log(
                                ...
                                % (
                                    common.to_str(self._remote_address[0]),
                ...
                if self._encrypt_correct:
                    ...
                    if obfs_decode[1]:
                        if not self._protocol.obfs.server_info.recv_iv:
                            ...
                            self._protocol.obfs.server_info.recv_iv = obfs_decode[0][
                                :iv_len
                            ]
    ...
    def _on_local_error(self):
        if self._local_sock:
            ...
            if err.errno not in [errno.ECONNRESET, errno.EPIPE]:
                logging.error(err)
                logging.error(
                    "local error, exception from %s:%d"
                    % (self._client_address[0], self._client_address[1])
                )
        self.destroy()
    ...
    def _on_remote_error(self):
        if self._remote_sock:
            ...
            if err.errno not in [errno.ECONNRESET]:
                ...
                if self._remote_address:
                    logging.error(
                        "remote error, when connect to %s:%d"
                        % (self._remote_address[0], self._remote_address[1])
                    )
                else:
                    logging.error(
                        "remote error, exception from %s:%d"
                        % (self._client_address[0], self._client_address[1])
                    )
        self.destroy()
    ...
    def handle_event(self, sock, fd, event):
        ...
        if fd == self._remote_sock_fd or fd == self._remotev6_sock_fd:
            if event & eventloop.POLL_ERR:
                ...
                if (
                    not self.speed_tester_d.isExceed()
                    and not self._server.speed_tester_d(self._user_id).isExceed()
                ):
                    handle = True
            ...
            if event & eventloop.POLL_ERR:
                ...
                if (
                    not self.speed_tester_u.isExceed()
                    and not self._server.speed_tester_u(self._user_id).isExceed()
                ):
                    handle = True
                    self._on_local_read()
                else:
...
class TCPRelay(object):
    def __init__(
        self, config, dns_resolver, is_local, stat_callback=None, stat_counter=None
    ):
        self._config = config
        self._is_local = is_local
        self._dns_resolver = dns_resolver
    ...
    def get_users_ud(self):
        return (
            self.server_user_transfer_ul.copy(),
            self.server_user_transfer_dl.copy(),
        )
    ...
    def stat_add(self, local_addr, val):
        if self._stat_counter is not None:
            ...
            self.update_stat(
                self._listen_port, self._stat_counter[self._listen_port], val
            )
            if newval <= 0:
            ...
            if newval >= self._stat_counter.get(-1, 0) + connections_step:
                ...
                self._stat_counter[-1] = (
                    self._stat_counter.get(-1, 0) + connections_step
                )
