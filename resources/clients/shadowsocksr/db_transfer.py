...
class TransferBase(object):
        ...
        for id in curr_transfer.keys():
            ...
            if id in self.last_get_transfer:
                if (
                    ...
                    > self.last_get_transfer[id][0] + self.last_get_transfer[id][1]
                ):
                    self.onlineuser_cache[id] = (
                        curr_transfer[id][0] + curr_transfer[id][1]
                    )
            else:
    ...
    def del_server_out_of_bound_safe(self, last_rows, rows):
        ...
        for row in rows:
            try:
                allow = (
                    switchrule.isTurnOn(row)
                    and row["enable"] == 1
                    and row["u"] + row["d"] < row["transfer_enable"]
                )
            except Exception as e:
                allow = False
            ...
            if allow:
                ...
                if (
                    ...
                    for name in merge_config_keys:
                        if (name in cfg) and (
                            (name not in relay._config)
                            or not self.cmp(cfg[name], relay._config[name])
                        ):
                            cfgchange = True
                            break
            ...
            if port in mu_servers:
                ...
                if ServerPool.get_instance().server_is_run(port) > 0:
                    ...
                    not config["additional_ports_only"]
                    and allow
                    and port > 0
                    and port < 65536
                    and ServerPool.get_instance().server_run_status(port) is False
                ):
                    self.new_server(port, passwd, cfg)
    ...
    def thread_db(obj):
        ...
        try:
            while True:
                ...
                if (
                    db_instance.event.wait(get_config().UPDATE_TIME)
...
class DbTransfer(TransferBase):
    ...
    def update_all_user(self, dt_transfer):
        ...
        for id in dt_transfer.keys():
            ...
            if (
                transfer[0] + transfer[1] < update_trs
                and id not in self.force_update_transfer
            ):
            ...
            query_sub_when += " WHEN %s THEN u+%s" % (
                id,
                int(transfer[0] * self.cfg["transfer_mul"]),
            )
            query_sub_when2 += " WHEN %s THEN d+%s" % (
                id,
                int(transfer[1] * self.cfg["transfer_mul"]),
            )
...
class Dbv3Transfer(DbTransfer):
    def __init__(self):
        ...
        self.update_node_state = (
            True if get_config().API_INTERFACE != "legendsockssr" else False
        )
                ...
                try:
                    if id in self.port_uid_table:
                        cur.execute(
                            ...
                            + "', '"
                            + str(self.cfg["transfer_mul"])
                            + "', '"
                            + self.traffic_format(
                                (transfer[0] + transfer[1]) * self.cfg["transfer_mul"]
                            )
                            + "', unix_timestamp()); "
                        )
                except:
            ...
            try:
                ...
                try:
                    cur.execute(
                        ...
                        + "` (`id`, `node_id`, `uptime`, `load`, `log_time`) VALUES (NULL, '"
                        + str(self.cfg["node_id"])
                        + "', '"
                        + str(self.uptime())
                        + "', '"
